...
parser.add_argument('-d', '--data', default='./data/ecg_data/datasets', type=str)
...
parser.add_argument('--schedule', type=int, nargs='+', default=[5,10,250,300,325,350], help='Decrease learning rate at these epochs.')
...
class Ecg_loader(Dataset):
    def __init__(self, ecg_dir, transform=None):
        """
        Args:
            metadata_path (str): CSV元数据文件路径
            ecg_dir (str): 存放.npy文件的目录
    ...
    def __getitem__(self, idx):
        ...
        l = row["label"]
        label = self.idx2name[l]

        # 加载ECG数据
        ecg_path = os.path.join(self.ecg_dir, filename)
        ecg = np.load(ecg_path).astype(np.float32)  # 确保为float32
        ...
        # 转换为PyTorch Tensor
        ecg_tensor = torch.from_numpy(ecg)  # shape: (12, time_steps)
...
def main():
    ...
    model = models.__dict__['cnn_lstm_transformer'](num_classes=num_classes,input_size = 12)
    print(type(model))
