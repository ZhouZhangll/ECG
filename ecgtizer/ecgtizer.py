...
from .PDF2XML_mod import plot_function
...
class ECGtizer:
    ...
    def __init__(self, file, dpi, Callback = None, extraction_method = "fragmented",typ = "", verbose = False, DEBUG = False):
        ...
        for image in images:
            ...
            self.all_image_clean.append(image_clean)
            if page == 0:
                self.df_patient = df
            image = image_clean
            if verbose == True:
    ...
    def get_extracted_lead(self,completion = False):
        if completion == False:
            return self.extracted_lead
        else:
            return self.extracted_lead_comp
